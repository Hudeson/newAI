...
| # | 遗漏项 | 说明 |
|---|--------|------|
| 9 | **用户组 / SCIM** | ACL 有 `group` principal，但缺 Group 实体与企业目录同步（SCIM） |
| 10 | **服务账号与 API Key** | 企业集成、连接器、CI 需要非人身份 |
| 11 | **通知渠道** | 学习完成/审批待办/任务失败：站内信、邮件、Webhook/Slack 未设计 |
| 12 | **会话与记忆策略** | 对话保留多久、可否跨会话记忆、敏感会话加密与清理 |
| 13 | **引用深链** | 点回 PDF 页码/段落锚点；仅有 chunk 不够产品化 |
| 14 | **反馈闭环** | 点赞/点踩、错误引用举报 → 进入评测集与重排特征 |
| 15 | **配额细化** | 仅有日 token/存储；缺并发 Worker 数、单次上传大小、空间级配额 |
| 16 | **审批流引擎边界** | 「approval」是内置简单状态机还是对接企业 BPM（飞书/钉钉/ServiceNow） |
| 17 | **连接器同步模型** | 增量游标、冲突（远端删除）、权限映射、同步背压 |
| 18 | **多语言 / i18n** | UI 与分词/嵌入对中英混合外的语言策略 |
| 19 | **Feature Flag** | 按租户灰度：自动学习、Agent、连接器 |

### 18.3 P2 — 企业加固期应补

| # | 遗漏项 | 说明 |
|---|--------|------|
| 20 | **DR：RPO/RTO** | 有备份手段，无恢复目标与演练频率 |
| 21 | **容量模型** | 文档量、QPS、向量维数 → 机器规格估算公式 |
| 22 | **网络隔离部署手册** | 纯离线/专有云安装、证书、模型包导入 |
| 23 | **法务与分包清单** | DPA、子处理方（LLM 厂商）、日志留存年限 |
| 24 | **Runbook** | 串租告警、队列打爆、模型全挂、密钥轮转操作手册 |
| 25 | **无障碍与设计系统** | 企业采购常审 a11y；前端规范未写 |
| 26 | **SDK / 公开 API 版本策略** | `/v1` 兼容、弃用窗口 |
| 27 | **计费** | 有 usage_ledger，无账单/发票/套餐（若走 SaaS） |
| 28 | **搜索可配置性** | 权重、过滤 DSL、同义词、租户级相关度调参 |
| 29 | **内容安全** | 病毒扫描钩子有了，缺恶意宏/加密 PDF/密码文件处理策略 |
| 30 | **Collection 实体** | 组织图提了 Collection，后文未展开与 Workspace/标签关系 |

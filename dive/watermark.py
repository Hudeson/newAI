...
* :func:`dive.decoding.generate` 会先跑 logits processor 再做温度缩放，
  所以有效偏置是 ``δ/T``。想让 δ 的含义与温度无关，就把水印放在温度之后。
* 水印藏在模型的"选择自由度"里。当下一个 token 几乎确定时（低熵），
  加再大的 δ 也改变不了输出，水印自然打不上——见 ``labs/lab05_watermark.py``
  的实验 D。
"""
